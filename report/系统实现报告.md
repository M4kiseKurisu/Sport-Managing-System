...
**前端**：采用 Vue 实现，包含 HTML，CSS 和 JS 的相关内容。同时调用了一些包来实现一些前端效果，例如大量使用了 Element Plus 包的组件。
...
├── UNMET DEPENDENCY @fortawesome/fontawesome-svg-core@^6.5.1
├── UNMET DEPENDENCY @fortawesome/free-solid-svg-icons@^6.5.1
├── UNMET DEPENDENCY @fortawesome/vue-fontawesome@^3.0.0-5
├── @vitejs/plugin-vue@4.4.0
├── axios@1.6.0
├── dayjs@1.11.10
├── echarts@5.4.3
├── element-plus@2.4.2
├── UNMET DEPENDENCY fortawesome@^0.0.1-security
├── sass@1.69.5
...
```
├─assets               存放基本样式
├─components           存放所有的vue文件（除了App.vue）
│  ├─Login             登陆注册相关界面
│  ├─MainPage          登陆之后的界面内容
│  │  ├─Components     可复用的组件信息
│  │  ├─Page  	       每个界面的总体文件，其中引用Components的组件（其中若一个板块有较多界面就再用文件夹区分）
│  │  │  ├─Activity    活动广场界面
│  │  │  ├─E-Borrow    器材租借界面
│  │  │  ├─Friend      好友界面
│  │  │  ├─Group       团体界面
...
合理使用数据库触发器可以防⽌产生异常数据，保证数据的完整性和一致性。

#### 3.1.1 删除触发器

本项目中很多联系表都有外键字段，因此删除实体时需要同时删除以此为外键的联系数据。

**删除团体**：删除团体同时删除团体成员表、活动创建表、团体申请表、团体器材借用表的数据。

```mysql
CREATE TRIGGER delete_group BEFORE DELETE ON api_group FOR EACH ROW
BEGIN
	DELETE FROM api_useringroup WHERE api_useringroup.gid = OLD.gid;
	DELETE FROM api_userapplygroup WHERE api_userapplygroup.gid = OLD.gid;
	DELETE FROM api_groupcreateactivity WHERE api_groupcreateactivity.gid = OLD.gid;
	DELETE FROM api_groupequipment WHERE api_groupequipment.gid = OLD.gid;
END
```

**删除活动**：以活动为外键的联系表非常多，删除活动需要同时删除对应的动态、场地使用记录、活动成员联系、活动创建记录。

```mysql
CREATE TRIGGER delete_activity BEFORE DELETE ON api_activity FOR EACH ROW
BEGIN
	DELETE FROM api_stream WHERE api_stream.activity = OLD.aid;
	DELETE FROM api_activityusefield WHERE api_activityusefield.aid = OLD.aid;
	DELETE FROM api_userinactivity WHERE api_userinactivity.aid = OLD.aid;
	DELETE FROM api_usercreateactivity WHERE api_usercreateactivity.aid = OLD.aid;
	DELETE FROM api_groupcreateactivity WHERE api_groupcreateactivity.aid = OLD.aid;
END
```

**删除动态**：删除动态需要同时删除用户点赞记录。

```mysql
CREATE TRIGGER delete_stream BEFORE DELETE ON api_stream FOR EACH ROW
BEGIN
	DELETE FROM api_userfavorstream WHERE api_userfavorstream.sid = OLD.sid;
END
```

#### 3.1.2 插入触发器

为了满足实际场景的约束，我们设置了插入触发器，检查数据的合理合法性，保证数据的有效性。

**插入团体**：团体的最大人数应该大于等于 1，不满足约束的数据不允许插入。

```mysql
CREATE TRIGGER insert_check_group
BEFORE INSERT ON api_group FOR EACH ROW
BEGIN
    IF NEW.maximum < 1 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Error: The max number of members must be at least 1';
    END IF;
END
```

**插入活动**：活动的最大参与人数应该大于等于1，不满足约束的数据不允许插入。

```mysql
CREATE TRIGGER insert_check_activity
BEFORE INSERT ON api_activity FOR EACH ROW
BEGIN
    IF NEW.maximum < 1 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Error: The max number of members must be at least 1';
...
**插入器材借用情况**：器材借用的最大数量不应大于当前该类器材的剩余数量。不满足约束的借用记录不被插入。

```mysql
-- 检查用户借用器材 --
CREATE TRIGGER insert_check_user_equipment
BEFORE INSERT ON api_userequipment
FOR EACH ROW
BEGIN
    DECLARE max_amount INT;

    SELECT amount INTO max_amount
    FROM api_equipment
    WHERE eid = NEW.eid;

    IF NEW.lend_amount > max_amount THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Error: Cannot borrow more equipment than is available';
    END IF;
END;

-- 检查团体借用器材 --
CREATE TRIGGER insert_check_group_equipment
BEFORE INSERT ON api_groupequipment
FOR EACH ROW
BEGIN
    DECLARE max_amount INT;
...
网站希望达成如果没有完成登录，就无法查看其它所有界面的功能，因此需要在每次跳转之前进行身份的验证，以路由守卫形式完成。

首先，在登陆时，会将用户的身份信息储存到全局变量中，可以在任意位置轻松的获取：
    ...
    if (this.account && this.password) {
        axios({
            ...
            if (result.data.status) {
                this.$store.commit("userLogin", result.data);  //将用户基本信息储存为全局变量
...
然后在进入登陆之外的界面时，都需要先查看是否存在用户信息全局变量，如果为空，证明当前没有成功登陆的信息，就无法查看这些界面，会自动跳转到登录注册界面。
...
其关键在于，每次跳转之前使用 `JSON.parse(sessionStorage.getItem())`的方法查找当前是否有储存的全局的用户信息。
...
- 左侧会展示团体的图片及名字。
- 右上侧展现了团体所承办的所有活动信息，并统计了这些活动的类型并用饼状图展现，还展示了近期的活动的基本信息，点击可以进入活动主页。
...
这次数据库大作业真的让我收获很多。首先，这是我作为一个六系“小透明”第一次承担组长这样有一定额外责任的职责；其次，前端也是一个我完全没有过接触的领域。因此，当我真正和两位组员一起努力，从头学起，把这个网站做出来了之后，获取的成就感也是无与伦比的。

整个网站前端的编写过程也很有意思。一开始我一腔热血，虽然完全不懂外部包的引入和使用别人写好的组件，但还是自己画组件，自己写界面，稍微将样式改的好看一点就觉得特别开心。但是中期遇到了很多自己写样式无法解决的问题，导致自己之前写的内容必须被弃用，当时又觉得非常灰心，有大概两周的时间什么都不想干。但是最后还是决心放弃了那些界面，重新开始，最后完成了这么多的任务。这个起-落-起的过程，回头看来也是感慨万千。

最后，不得不夸赞我的两个组员，华哥的后端一直非常稳定，接口的更新和维护也非常令人放心。锐哥的前端投入也很大，他也为了这个项目经常熬夜，非常努力。总而言之，如果没有两个组员这样的尽心尽力，我们是无法完成这个项目的。

**陈俊华**：

本次项目我们从前期分工到后期合作开发，整个过程都相当顺利，组内成员都很用心的投入到这次大作业里，可以算是一次很棒的小组作业了。

本项目中我担任后端开发的工作，主要负责了数据库的维护完善以及接口的编写。比起前端的繁琐，后端其实并不算特别复杂。但是后端需要考虑很多和数据库相关的问题，比如如何维护好数据的正确性，有效性。如果因为一点考虑不周，就会导致数据库数据的不统一，维护起来相当麻烦。此外，前期设计数据库时，由于没有课程理论做支撑，更多的仅凭应用需求设计数据库，到后期发现许多地方需要完善修改。有了数据库理论做支撑，我们的数据库设计起来更加有条理，也符合规范，这在我编写后端时深有体会。

回顾这个学期的开发，从前期的准备，学习基本技术包括 Django、Vue，到中期定时设定目标，定时开组会检查进度，以及后期美化界面，测试数据修改 bug，每一位成员都付出了同等的努力，这次大作业凝结了所有人的努力，见证了我们的成长。

于个人而言，这是我第一次小组任务，让我更加清楚如何与组内伙伴沟通，如何通力合作。看着一个精美的活动网站稳定地运行在界面中，内心更是油然而生的成就感。

**李子锐**：
本次小组合作我收获良多，首先是和两位非常积极、非常有实力的同学合作，让我深感荣幸，其次是课程内容让我深入了解了数据库和网站开发。

在本次任务中我负责前端部分功能的开发，这对我来说是一个完全没有接触过的领域。最开始编写前端时我无从入手，强烈依赖网络上的现有代码、chatgpt和组件，但团队最开始给了我充足的适应时间和较少的工作量，让我最后可以跟得上团队的脚步。由于起步较晚，我制作的网页常常观感较差、bug众多，两位同学总会帮我推荐工具，提出修改的idea，帮我完善我开发的网页的功能。同时我还完成了报告中流图、ER图、功能结构图等图片的制作，这也让我学到了很多，从最开始的一两张图需要磨一个晚上到后期的熟练使用绘图工具。虽然最后绘制的图片还是有很多的瑕疵，但这也是一个很不错的体验。

在开发过程中，遇到了很多挑战和困难，常常需要通宵达旦，甚至在展示前一刻还在调整代码。但是最终的成功展示让我感动和自豪。最后，我要再次感谢博哥和华哥对我的帮助，他们的付出让我能够顺利的完成完成了这个庞大的团队项目。非常感谢这次宝贵的机会，不仅教会了我技术方面的知识，也锻炼了我的团队合作能力和抗压能力。
