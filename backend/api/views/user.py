...
def login(request):
    if request.method == 'GET':
        ...
        if user:
            return JsonResponse({"msg": '登录成功', "res": True, "id": user.uid, "user_name": user.user_name})
        else:
            return JsonResponse({"msg": '登录失败', "res": False, "id": '000000', "user_name": None})
    else:
...
def register(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(account=account).first():
            return JsonResponse({"msg": "账号已被注册", "id": "", "user_name": ""})
        elif User.objects.filter(Q(phone_number=phone_number)|Q(email=email)).first():
            return JsonResponse({"msg": "电话或邮箱已被注册", "id": "", "user_name": ""})
        else:
            uid = genid()
            ...
            ps = data.get("personal_signature")
            new_user = User(uid=uid, account=account, password=password, user_name=user_name,
                            ...
                            email=email, personal_signature=ps)
            new_user.save()
            return JsonResponse({"msg": "注册成功", "id": uid, "user_name": user_name})
    else:
        return JsonResponse({"msg": "请求方式有误"})
